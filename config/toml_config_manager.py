...
def validate_env(*, env: str | None) -> ValidEnvs:
    if env is None or env not in ValidEnvs:
        ...
        raise ValueError(
            f"Environment variable {ENV_VAR_NAME} has invalid value: {env_display}. Must be one of: {valid_values}."
        )
...
def get_env_value_by_export_field(*, config: dict[str, Any], field: str) -> Any:
    ...
    if isinstance(current, (dict, list)):
        raise ValueError(f"Field '{field}' cannot be converted to string: got {type(current).__name__}")
    try:
        return str(current)
    except (TypeError, ValueError) as e:
        raise ValueError(f"Field '{field}' cannot be converted to string: {str(e)}") from e
...
def write_dotenv_file(*, env: ValidEnvs, exported_fields: dict[str, str]) -> None:
    env_filename = f"{DirContents.DOTENV_NAME}"
