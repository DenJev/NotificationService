...
class AuthSettings(BaseModel):
    ...
    def validate_session_refresh_threshold(cls, v: Any) -> float:
        if isinstance(v, (int, float)):
            if not 0 < v < 1:
                raise ValueError("SESSION_REFRESH_THRESHOLD must be between 0 and 1, exclusive.")
            return v
        else:
            raise ValueError(
                "SESSION_REFRESH_THRESHOLD must be a number (fraction, 0 < fraction < 1)."  # pylint: disable=implicit-str-concat
